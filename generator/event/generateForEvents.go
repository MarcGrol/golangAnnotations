...
func Apply{{$aggr}}Event(c context.Context, envelope eventStore.Envelope, aggregateRoot {{$aggr}}Aggregate) error {
...
func Apply{{$aggr}}Events(c context.Context, envelopes []eventStore.Envelope, aggregateRoot {{$aggr}}Aggregate) error {
	var err error
...
func UnWrap{{$aggr}}Event(envelope *eventStore.Envelope) (events.Event, error) {
	switch envelope.EventTypeName {
	{{range $aggregName, $eventName := $events}}
	case {{$eventName}}EventName:
		event, err := UnWrap{{$eventName}}(envelope)
...
func UnWrap{{$aggr}}Events(envelopes []eventStore.Envelope) ([]events.Event, error) {
...
func (s *{{.Name}}) Wrap(sessionUID string) (*eventStore.Envelope,error) {
...
func Is{{.Name}}(envelope *eventStore.Envelope) bool {
...
func GetIfIs{{.Name}}(envelope *eventStore.Envelope) (*{{.Name}}, bool) {
...
func UnWrap{{.Name}}(envelope *eventStore.Envelope) (*{{.Name}},error) {
    if Is{{.Name}}(envelope) == false {

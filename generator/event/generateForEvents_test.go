...
func TestGenerateForEvents(t *testing.T) {
	...
	assert.Contains(t, string(data), "func ApplyTestEvent(c context.Context, envelope eventStore.Envelope, aggregateRoot TestAggregate) error {")
	assert.Contains(t, string(data), "func ApplyTestEvents(c context.Context, envelopes []eventStore.Envelope, aggregateRoot TestAggregate) error {")
	assert.Contains(t, string(data), "func UnWrapTestEvent(envelope *eventStore.Envelope) (events.Event, error) {")
	assert.Contains(t, string(data), "func UnWrapTestEvents(envelopes []eventStore.Envelope) ([]events.Event, error) {")

	// check that generate code has 4 helper functions for MyStruct
	data, err = ioutil.ReadFile("./testData/$wrappers.go")
	assert.NoError(t, err)
	assert.Contains(t, string(data), "func (s *MyStruct) Wrap(sessionUID string) (*eventStore.Envelope,error) {")
	assert.Contains(t, string(data), "func IsMyStruct(envelope *eventStore.Envelope) bool {")
	assert.Contains(t, string(data), "func GetIfIsMyStruct(envelope *eventStore.Envelope) (*MyStruct, bool) {")
	assert.Contains(t, string(data), "func UnWrapMyStruct(envelope *eventStore.Envelope) (*MyStruct,error) {")

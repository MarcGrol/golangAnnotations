...
func TestGenerateForWeb(t *testing.T) {
	...
	assert.Contains(t, string(data), `func (es *MyEventService) handleEvent(c context.Context, credentials rest.Credentials, topic string, envlp envelope.Envelope) error{`)
}
